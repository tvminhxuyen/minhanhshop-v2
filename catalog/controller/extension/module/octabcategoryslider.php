...
class ControllerExtensionModuleOctabcategoryslider extends Controller {
	public function index($setting) {
		...
		if(isset($setting['image1']) && $setting['image1']) {
			$cover_image1 =  $this->model_tool_image->resize($setting['image1'], 460, 200);
		} else {
			$cover_image1 = false;
		}
		
		if(isset($setting['image2']) && $setting['image2']) {
			$cover_image2 =  $this->model_tool_image->resize($setting['image2'], 650, 200);
		} else {
			$cover_image2 = false;
		}
